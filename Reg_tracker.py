# -*- coding: utf-8 -*-
"""
Created on Tue Feb  4 17:49:57 2020
...
import re, sys, xlrd, xlwt, os, argparse
...
def ExcelDetectPoll():
    ...
    nrows = range(sheet1.nrows)
    for row in nrows:
        if sheet1.row(row)[0].value == "RD": col_a.append(sheet1.row(row)[1].value)
    counts = Counter(col_a)
    RdPoll = [item for item, count in counts.items() if count >= 2]
    return RdPoll

def ExcelExport(samples, pre, wb): #Write to Excel Sheet
    sheet1 = wb.add_sheet('Sheet 1') 
    for i in range(0,len(samples)):
        ap  = (samples[i].split("|")[8].strip())
        if ap == "AP":
            reg   = (pre + samples[i].split("|")[4].strip())
            val   = (samples[i].split("|")[7].strip()[-2:])
            typ   = (samples[i].split("|")[5].strip())
            delay = int(samples[i].split("|")[9].strip())
            sheet1.write(i, 0, typ) 
            sheet1.write(i, 1, reg) 
            sheet1.write(i, 2, val) 
            if  delay != 0 : sheet1.write(i, 3, delay) 
            else: sheet1.write(i, 3, 0) 
    wb.save('Data.xls') 
...
def RegTransaction(f, wb, pre):
    ...
    with open(args.file, encoding="utf8") as inputfile:
        ...
        ExcelExport(samples, pre, wb)   #Excel Not Necessary
        RdPoll = ExcelDetectPoll()      #Excel Not Necessary
        print(RdPoll)
